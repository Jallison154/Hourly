...
export default function Navigation() {
  ...
  const navItems = [
    ...
    { path: '/', label: 'Clock', match: ['/', '/clock'] },
    { path: '/timesheet', label: 'Timesheet' },
    { path: '/schedule', label: 'Schedule' },
    { path: '/paycheck', label: 'Paycheck' },
  ]
  ...
  return (
    <nav className="hidden sm:block bg-okami-panel shadow-panel border-b border-okami-border">
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium min-h-[44px] ${
                    isActive(item)
                      ? 'border-okami-accent text-gray-900 dark:text-white'
