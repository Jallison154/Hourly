...
import { timesheetAPI, timeEntriesAPI, paycheckAPI, userAPI } from '../services/api'
...
export default function Timesheet() {
  const [activeTab, setActiveTab] = useState<'timesheet' | 'paycheck'>('timesheet')
  ...
  const handleSaveEdit = async (updates: { clockIn: string; clockOut: string | null; notes?: string | null }) => {
    ...
    try {
      ...
      if (activeTab === 'timesheet' && selectedPeriod) {
        await loadTimesheet(selectedPeriod.start, selectedPeriod.end)
      } else if (activeTab === 'paycheck' && useCurrentPeriod && selectedPeriod) {
  ...
  const loadPayPeriodsForPaycheck = async () => {
    try {
      const periods = await timesheetAPI.getPayPeriods()
      setPayPeriods(periods)
  ...
  // Load user profile and pay periods when switching to paycheck tab
  useEffect(() => {
    if (activeTab === 'paycheck') {
      loadUserProfile()
      loadPayPeriodsForPaycheck()
    }
  }, [activeTab])

  // Load estimate when pay period changes in paycheck tab
  useEffect(() => {
    if (activeTab === 'paycheck' && useCurrentPeriod && selectedPeriod) {
      loadPeriodEstimate(selectedPeriod.start, selectedPeriod.end)
    }
  }, [activeTab, useCurrentPeriod, selectedPeriod])
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:pb-8" style={{ paddingBottom: 'calc(4rem + env(safe-area-inset-bottom))' }}>
                  ...
                  >
                    ...
                    {calculation.weeklyBreakdown && calculation.weeklyBreakdown.length > 0 && (
                      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                        ...
                        <div className="space-y-4">
                          {calculation.weeklyBreakdown.map((week: any) => (
                            <motion.div
                              key={week.weekNumber}
      ...
      {/* Week Breakdown Modal - Shared between tabs */}
      {selectedWeek && (
        <div 
            ...
            <div className="space-y-4">
              ...
              {selectedWeek.entries && selectedWeek.entries.length > 0 && (
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    {selectedWeek.entries.map((entry: any) => (
                      <motion.div
                        key={entry.id}
                        onClick={async () => {
                          try {
      ...
      {/* Edit/Create Entry Modal - Shared between tabs */}
      {editingEntry && (
